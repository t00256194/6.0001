...
def get_word_score(word, n):
    ...
    returns: int >= 0
    """
    word = list(word.lower())   # keys in SCRABBLE_LETTER_VALUES are lowercase
    wordlen = len(word)
    points = []
    for letter in word:
        if letter == '*':
...
def update_hand(hand, word):
    ...
    Letters that appear in word more times than in hand should never result
    in a negative count; instead, set the count in the returned hand to 0
    ...
    if word in word_list:
        for letter in word:
            ...
            if word.count(letter) > hand[letter]:
                ...
                break
        return True
    elif '*' in word:           # wildcard case
        for vowel in VOWELS:
            check_word = word.replace('*', vowel.lower())
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    if n <= 0:
        ...
        print('Total for this hand:', total)
    return total
...
def substitute_hand(hand, letter):
    ...
    if letter not in hand:
        return hand
    copy = hand.copy()
    sub = random.choice([s for s in string.ascii_lowercase if s != letter and s not in hand.keys()])
    copy[sub] = copy.pop(letter) 
    return copy
