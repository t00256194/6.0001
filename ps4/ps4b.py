...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for shift in range(26):
            shifted_message = self.apply_shift(shift).split()   
            matches = 0
            
            # Add 1 for each valid word in shifted_message
            # Add number of matches for each shift value to valid_word_count
            for word in shifted_message:                         
                if is_word(self.get_valid_words(), word):        
                    matches += 1
            valid_word_count.append(matches)
            
        # Find index of max value in matches (shift value)
        best_shift = valid_word_count.index(max(valid_word_count))
        decrypted_message = self.apply_shift(best_shift - 26)
